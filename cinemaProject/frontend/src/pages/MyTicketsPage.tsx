...
const MyTicketsPage = () => {
  ...
  const handleCancelBooking = async (bookingId: number) => {
    ...
    try {
      ...
      // Move booking to cancelled state locally
      setBookings((prev) =>
        prev.map((b) =>
          b.id === bookingId ? { ...b, status: "CANCELLED" } : b
        )
      );
      setActionMessage({ type: "success", text: "Booking cancelled and seats released." });
    } catch (err: any) {
      const msg = err?.message || "Unable to cancel this booking. Please try again.";
